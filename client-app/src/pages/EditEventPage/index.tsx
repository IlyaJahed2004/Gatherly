...
const EditEventPage = observer(() => {
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof EditEventForm | 'submit', string>>>({});
  const [isSubmitting, setIsSubmitting] = useState(false);

  // ── Load event and pre-fill form ──
  useEffect(() => {
    if (!id) return;
    agent.Events.details(id)
      .then((data) => {
        setEvent(data);
        const start = splitISO(data.startDate);
        const end = splitISO(data.endDate);
        setForm({
          title: data.title,
  ...
  // ── Handlers ──
  const set = (key: keyof EditEventForm, value: string | number | boolean) =>
  ...
  if (!event.isHost) {
    return (
      <div className="flex items-center justify-center py-40">
        <span className="text-gray-400 text-[20px]">You're not authorized to edit this event.</span>
      </div>
    );
  }

  const currentImageUrl = deleteImage ? null : (imagePreview ?? event.imageUrl);

  return (
    <div className="min-h-screen bg-[#F3F4F6] py-10 px-6 lg:px-16">
      <div className="max-w-[1100px] mx-auto">
        {/* Title */}
        <h1 className="text-[32px] font-semibold text-[#078C80] mb-8">Edit Event</h1>
